...
#I need only one file to be backed up at a time, so I am doing the following paragraph of code
if [ $# -ne 1 ]; then
    echo "Usage: $0 <hint:enter the directory name to be copied ie., $HOME/RAW_DATA $HOME/P_DATA>"
    exit 1
fi
...
for fn in "$source_dir"/*; do
    if [ -f "$fn" ]; then
        ...
        echo "Copied: $fn to $HOME/BACKUP_FILES"
        ((total_files++))
    fi
done
...
print_info () {
    echo "File information in $HOME/BACKUP_FILES:"
